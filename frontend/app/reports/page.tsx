    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 rounded-lg text-sm ${tab === t.key ? 'bg-blue-600 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'movement' && (
        <div className="flex gap-4 mb-4">
          ...
          <button onClick={fetchData} className="bg-blue-600 text-white px-4 py-2 rounded-lg">Filter</button>
        </div>
      )}
        ...
        {loading ? <div className="p-6 text-center">Loading...</div> : (
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {data.length === 0 && <tr><td colSpan={5} className="py-4 text-center text-gray-500">No data</td></tr>}
            </tbody>
          </table>
        )}
      </div>
